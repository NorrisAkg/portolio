...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (body.action === 'publish') {
      const useCase = container.resolve<PublishArticleUseCase>('PublishArticleUseCase');
      for (const id of body.ids) {
        await useCase.execute({ id });
      }
    } else if (body.action === 'unpublish') {
      const useCase = container.resolve<UnpublishArticleUseCase>('UnpublishArticleUseCase');
      for (const id of body.ids) {
        await useCase.execute({ id });
      }
    } else if (body.action === 'delete') {
      const useCase = container.resolve<DeleteArticleUseCase>('DeleteArticleUseCase');
      for (const id of body.ids) {
        await useCase.execute({ id });
      }
    }
